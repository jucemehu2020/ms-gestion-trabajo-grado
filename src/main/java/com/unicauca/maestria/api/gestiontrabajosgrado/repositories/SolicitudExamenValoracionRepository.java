...
public interface SolicitudExamenValoracionRepository extends JpaRepository<SolicitudExamenValoracion, Long> {
    ...
    @Query("SELECT COUNT(sev) FROM SolicitudExamenValoracion sev WHERE sev.trabajoGrado.id = ?1")
    ...
    @Query("SELECT CASE WHEN COUNT(sev) > 0 THEN TRUE ELSE FALSE END FROM SolicitudExamenValoracion sev WHERE sev.trabajoGrado.id = ?1")
    public boolean existsByTrabajoGradoId(Long trabajoGradoId);

    @Query("SELECT sev.id FROM SolicitudExamenValoracion sev WHERE sev.trabajoGrado.id = ?1")
